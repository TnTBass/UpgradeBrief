...
const catalog = {
  ...
  releases: [{ id: 'vbr-12-3-2-3617', productId: 'vbr', aliases: ['12.3.2.3617'], sourceIds: ['kb4771'] }, ...families.map((family) => ({ id: `vbr-${family.replace(/\./g, '-')}`, productId: 'vbr', aliases: [family, `${family}.1`], sourceIds: ['kb2680'] })), { id: 'vbr-13-1-1', productId: 'vbr', aliases: ['13.1.1.18', '13.1.1'], sourceIds: ['kb2680'] }],
  upgradePaths: [
...
if (merged.paths !== 11 || direct?.toReleaseId !== 'vbr-13-1-1' || direct.hopReleaseIds.join() !== 'vbr-13-1-1') throw new Error('VBR KB2053 merge did not map the documented 13.1 family to its current recommended patch.')
...
if (exactBuildRoute?.toReleaseId !== 'vbr-13-1-1' || exactBuildRoute.hopReleaseIds.join() !== 'vbr-12-3-2-4165,vbr-13-1-1') throw new Error('VBR KB2053 merge must advance exact-build guidance while retaining its security prerequisite.')
